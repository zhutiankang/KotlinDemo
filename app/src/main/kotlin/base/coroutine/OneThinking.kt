...
//协程虽然运行在线程之上，但协程并不会和某个线程绑定，在某些情况下，协程是可以在不同的线程之间切换的 挂起和恢复

//协程对比线程还有一个特点，那就是非阻塞  挂起和恢复
...
Kotlin 协程的“非阻塞”其实只是语言层面的, Thread.sleep() 的时候，它仍然会变成阻塞式的  尽量使用 delay，而不是 sleep。
...
协程是如何通过挂起和恢复来实现非阻塞的 大部分的语言当中都会存在一个类似“调度中心”的东西，它会来实现 Task 任务的执行和调度
